...
void GUI_v2::setupSavegameNames(Menu &menu, int num) {
	...
	for (int i = startSlot; i < num && uint(_savegameOffset + i) < _saveSlots.size(); ++i) {
		if ((in = _vm->openSaveForReading(_vm->getSavegameFilename_test(_saveSlots[i + _savegameOffset]).c_str(), header)) != 0) {
...
int GUI_v2::deleteMenu(Button *caller) {
	...
	initMenu(*_currentMenu);
	updateAllMenuButtons();
	_vm->_saveFileMan->removeSavefile(_vm->getSavegameFilename_test(_slotToDelete).c_str());
	...
	while (i != _saveSlots.end()) {
		...
		if (*(i - 1) != *i || *i >= 990)
			break;
		Common::String oldName = _vm->getSavegameFilename_test(*i).c_str();
		Common::String newName = _vm->getSavegameFilename_test(*i - 1).c_str();
