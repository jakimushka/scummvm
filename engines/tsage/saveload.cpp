...
Common::Error Saver::save(int slot, const Common::String &saveName) {
	...
	Common::OutSaveFile *saveFile = g_system->getSavefileManager()->openForSaving(g_vm->getSavegameFilenameTemp(g_vm->getTargetName(), slot));
	if (!saveFile)
...
Common::Error Saver::restore(int slot) {
	...
	// Set up the serializer
	Common::InSaveFile *saveFile = g_system->getSavefileManager()->openForLoading(g_vm->getSavegameFilenameTemp(g_vm->getTargetName(), slot));
	if (!saveFile)
...
bool Saver::savegamesExist() const {
	Common::String slot1Name = g_vm->getSavegameFilenameTemp(g_vm->getTargetName(), 1);
